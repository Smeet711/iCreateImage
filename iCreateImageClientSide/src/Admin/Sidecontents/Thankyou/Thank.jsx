...
function Thank() {
  const [thankyouData, setData] = useState([]);
  const type = "thankyou";
  const [fromDate, setFromDate] = useState("");
  const [toDate, setToDate] = useState("");

  useEffect(() => {
    fetch("https://icreate-user-backend.onrender.com/api/thankyou-sub-brand")
      .then((res) => res.json())
      .then((data) => setData(data));

    console.log("data: ", thankyouData);
  }, []);
        ...
        <div className="w-full">
          <div className="h-[135px] rounded-xl grid place-items-center bg-purple-700 text-white">
            <div>
              ...
              <h1 className="text-4xl">{thankyouData.ThankyouCount}</h1>
            </div>
          </div>

          <div className="grid grid-cols-3 pt-10 gap-4">
            {thankyouData.subBrands &&
              thankyouData.subBrands.map((item, index) => (
                <div
                  key={index}
