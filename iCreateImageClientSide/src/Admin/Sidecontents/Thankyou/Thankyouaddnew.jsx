...
function Thankyouaddnew() {
  ...
  const fetchBrandName = async () => {
    try {
      const response = await fetch("https://icreate-admin-backend.onrender.com/thankyous/brandname");
  ...
  const submit = async (e) => {
    ...
    try {
      const response = await axios.post(
        ...
        "https://icreate-admin-backend.onrender.com/thankyous/new-design",
        formData,
        {
          headers: {
 ...
 if (selectedBrand) {
   navigate(`/Admin/thankyou-sub-brands/${selectedBrand._id}`);
 }
  ...
  const handleLogoSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://icreate-admin-backend.onrender.com/logo",
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
      ...
      console.log("error: ", response.data);
      if (response.data.message === "Logo Created successfully") {
        ...
        <div>
          {successMessage && (
            <p className="text-center text-green-500 text-xl  font-bold">
              ThankYou design Created successfully
            </p>
          )}

          {errorMessage && (
            <p className="text-center text-red-500 text-xl  font-bold">
              Error creating the ThankYou design
            </p>
          )}
        </div>
      </div>
