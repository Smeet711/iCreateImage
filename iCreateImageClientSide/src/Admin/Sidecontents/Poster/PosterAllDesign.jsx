...
const PosterAllDesign = () => {
  const { id } = useParams();
  const [PostersDesign, setPostersDesign] = useState({});

  useEffect(() => {
    fetch(`http://localhost:3000/posters-brands/${id}`, {
      method: "GET",
    })
      .then((res) => res.json())
      .then((data) => setPostersDesign(data));
  }, []);

  console.log(PostersDesign);
  ...
  return (
    <div>
      ...
      <div className="grid md:grid-cols-4 sm:grid-cols-2 gap-7">
        {PostersDesign.design &&
          PostersDesign.design.map((item, index) => (
            <div>
              <div
                key={index}
...
export default PosterAllDesign;
