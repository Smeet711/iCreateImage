...
function Poster() {
  ...
  useEffect(() => {
    fetch("http://localhost:3000/api/poster-sub-brand")
      .then((res) => res.json())
  ...
  <div className="w-full max-w-4xl bg-white p-8 rounded-lg shadow-lg">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
      <div className="h-[200px] rounded-lg grid place-items-center bg-[#3366ff] text-white">
        <div>
          <h1 className="text-lg font-semibold">Poster</h1>
          <h1 className="text-4xl">{posterData.posterCount}</h1>
        </div>
      </div>

      {posterData.subBrands &&
        posterData.subBrands.map((item, index) => (
          <div
            key={index}
            className="h-[200px] rounded-lg grid place-items-center "
