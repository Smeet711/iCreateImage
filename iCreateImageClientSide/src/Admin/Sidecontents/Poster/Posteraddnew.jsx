...
//     formData.append("designName", design);
//     formData.append("image", image.file, image.name);
//     try {
//       const response = await axios.post(
//         "https://icreate-admin-backend.onrender.com/posters",
//         formData,
//         {
//           headers: {
...
//       if (response.data.message === "Poster created successfully") {
...
const Posteraddnew = () => {
  ...
  const fetchBrandName = async () => {
    try {
      const response = await fetch("http://localhost:3000/posters/brandname");
  ...
  const submit = async (e) => {
    ...
    try {
      const response = await axios.post(
        ...
        "http://localhost:3000/posters/new-design",
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
      ...
      if (response.data.msg === "Design added successfully") {
        ...
        setErrorMessage(false); // Clear any previous error message
         ...
         if (selectedBrand) {
           navigate(`/Admin/poster-sub-brands/${selectedBrand._id}`);
         }
        ...
        <div className=" w-[100%]">
          ...
          <div>
            <div>
              {successMessage && (
                <p className="text-center text-green-500 text-xl  font-bold">
                  Poster Design created successfully
                </p>
              )}

              {errorMessage && (
                <p className="text-center text-red-500 text-xl  font-bold">
                  Error creating the Poster Design
                </p>
              )}
            </div>
          </div>
