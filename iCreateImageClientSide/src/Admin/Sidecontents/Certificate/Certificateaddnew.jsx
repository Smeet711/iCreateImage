  ...
  const fetchBrandName = async () => {
    try {
      const response = await fetch(
        "https://icreate-admin-backend.onrender.com/certificates/brandname"
      );
  ...
  const submit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://icreate-admin-backend.onrender.com/certificates/new-design",
        formData,
        {
          headers: {
      ...
      if (response.data.msg === "Design added successfully") {
        ...
        setErrorMessage(false); // Clear any previous error message
         ...
         if (selectedBrand) {
           navigate(`/Admin/certificate-sub-brands/${selectedBrand._id}`);
         }
          ...
          <div>
            {successMessage && (
              <p className="text-center text-green-500 text-xl  font-bold">
                Certificate Design created successfully
              </p>
            )}

            {errorMessage && (
              <p className="text-center text-red-500 text-xl  font-bold">
                Error creating the Certificate Design
              </p>
            )}
          </div>
        </div>
...
export default Certificateaddnew;
