...
const CertificatesAllDesign = () => {
  const { id } = useParams();
  const [CertificateDesign, setCerificateDesign] = useState({});
  ...
  useEffect(() => {
    fetch(`https://icreate-admin-backend.onrender.com/certificates-brands/${id}`, {
      ...
      .then((res) => res.json())
      .then((data) => setCerificateDesign(data));
  }, []);
...
export default CertificatesAllDesign;
