...
function ThankYouReport() {
  ...
  function handleFilter() {
    if (fromDate && toDate && fromDate <= toDate) {
      ...
      const filteredData = cnt.allThankYouData.map((item) => {
        const filteredthankyou = item.thankYouItems.filter((thankyou) => {
          const itemDate = new Date(thankyou.dateOfCreation).getTime();
        ...
        console.log({
          ...
          thankyouCount: filteredthankyou.length,
          filteredthankyou,
        });

        return { ...item, thankYouItems: filteredthankyou };
      });

      console.log({ filteredData });

      const filteredDataWithThankYou = filteredData.filter(
        (item) => item.thankYouItems.length > 0
      );

      allCnt({ ...cnt, allThankYouData: filteredDataWithThankYou });
    }
  }

  useEffect(() => {
    console.log("fetching....");
    fetch("https://icreate-user-backend.onrender.com/api/users-thankyou-data")
      .then((res) => res.json())
  ...
  function downloadExcel() {
    ...
    const worksheet = XLSX.utils.json_to_sheet(
      cnt.allThankYouData.flatMap((thankYouData) =>
        thankYouData.thankYouItems.map((thankYouItem) => ({
          EmpName: thankYouData.user.USERNAME,
          EmpID: thankYouData.user.MRID,
          Email: thankYouData.user.EMAIL,
          MRRole: thankYouData.user.ROLE,
        }))
      )
    );

    XLSX.utils.book_append_sheet(workbook, worksheet, "ThankYouData");
    ...
    const fileName = "thankyou_data.xlsx";
          ...
          <div className="w-[100%] ">
            <table className="border rounded-t-lg">
              ...
              <tbody className="table-box">
                {cnt.allThankYouData &&
                  cnt.allThankYouData.length > 0 &&
                  cnt.allThankYouData.map((thankyouData) =>
                    thankyouData.thankYouItems.map((thankyou) => (
                      <tr key={thankyou._id}>
                        <td className="capitalize">
                          {thankyouData.user.USERNAME}
                        </td>
                        <td>{thankyouData.user.MRID}</td>
                        <td>{thankyouData.user.EMAIL}</td>
                        <td>{thankyouData.user.ROLE}</td>
                        <td>{thankyouData.user.HQ}</td>
                        <td>{thankyouData.user.REGION}</td>
                        <td>{thankyouData.user.BUSINESSUNIT}</td>
                        <td>{thankyouData.user.DOJ}</td>
                        <td>{thankyouData.user.SCCODE}</td>
                        <td>{thankyou.brandName}</td>
                        <td>{thankyou.drName}</td>
                        <td>{formatDateString(thankyou.dateOfCreation)}</td>
                        {/* <td>{poster.topic}</td> */}

                        <td>{thankyouData.thankYouItems.length}</td>
                        <td>{0}</td>
                        <td>
