...
function BirthdayReport() {
  ...
  function handleFilter() {
    if (fromDate && toDate && fromDate <= toDate) {
      ...
      const filteredData = cnt.allBirthdayData.map((item) => {
        const filteredbirthdays = item.birthdays.filter((birth) => {
          const itemDate = new Date(birth.dateOfCreation).getTime();
        ...
        console.log({
          ...
          birthCount: filteredbirthdays.length,
          filteredbirthdays,
        });

        return { ...item, birthdays: filteredbirthdays };
      });

      console.log({ filteredData });

      const filteredDataWithbirthdays = filteredData.filter(
        (item) => item.birthdays.length > 0
      );

      allCnt({ ...cnt, allBirthdayData: filteredDataWithbirthdays });
    }
  }

  useEffect(() => {
    console.log("fetching....");
    fetch("https://icreate-user-backend.onrender.com/api/users-birthday-data")
      .then((res) => res.json())
  ...
  function downloadExcel() {
    ...
    const worksheet = XLSX.utils.json_to_sheet(
      cnt.allBirthdayData.flatMap((birthdayData) =>
        birthdayData.birthdays.map((birthday) => ({
          EmpName: birthdayData.user.USERNAME,
          EmpID: birthdayData.user.MRID,
          Email: birthdayData.user.EMAIL,
          MRRole: birthdayData.user.ROLE,
        }))
      )
    );

    XLSX.utils.book_append_sheet(workbook, worksheet, "BirthdayData");
    ...
    const fileName = "birthday_data.xlsx";
          ...
          <div className="w-[100%] ">
            <table className="border rounded-t-lg">
              ...
              <tbody className="table-box">
                {cnt.allBirthdayData &&
                  cnt.allBirthdayData.length > 0 &&
                  cnt.allBirthdayData.map((birthData) =>
                    birthData.birthdays.map((birthday) => (
                      <tr key={birthday._id}>
                        <td className="capitalize">
                          {birthData.user.USERNAME}
                        </td>
                        <td>{birthData.user.MRID}</td>
                        <td>{birthData.user.EMAIL}</td>
                        <td>{birthData.user.ROLE}</td>
                        <td>{birthData.user.HQ}</td>
                        <td>{birthData.user.REGION}</td>
                        <td>{birthData.user.BUSINESSUNIT}</td>
                        <td>{birthData.user.DOJ}</td>
                        <td>{birthData.user.SCCODE}</td>
                        <td>{birthday.brandName}</td>
                        <td>{birthday.drName}</td>
                        <td>{formatDateString(birthday.dateOfCreation)}</td>
                        {/* <td>{poster.topic}</td> */}

                        <td>{birthData.birthdays.length}</td>
                        <td>{0}</td>
                        <td>
