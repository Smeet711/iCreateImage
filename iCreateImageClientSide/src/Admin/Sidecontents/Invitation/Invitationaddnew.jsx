  ...
  const fetchBrandName = async () => {
    try {
      const response = await fetch(
        "https://icreate-admin-backend.onrender.com/invitations/brandname"
      );
  ...
  const submit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://icreate-admin-backend.onrender.com/invitations/new-design",
        formData,
        {
          headers: {
      ...
      if (selectedBrand) {
        navigate(`/Admin/invitation-sub-brands/${selectedBrand._id}`);
      }
            ...
            <div>
              {successMessage && (
                <p className="text-center text-green-500 text-xl  font-bold">
                  Invitation Design created successfully
                </p>
              )}

              {errorMessage && (
                <p className="text-center text-red-500 text-xl  font-bold">
                  Error creating the invitation Design
                </p>
              )}
            </div>
          </div>
...
export default Invitationaddnew;
