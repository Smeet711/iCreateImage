...
const InvitationsAllDesign = () => {
  const { id } = useParams();
  const [InvitationDesign, setInvitationDesign] = useState({});

  useEffect(() => {
    fetch(`https://icreate-admin-backend.onrender.com/invitations-brands/${id}`, {
      method: "GET",
    })
      .then((res) => res.json())
...
export default InvitationsAllDesign;
