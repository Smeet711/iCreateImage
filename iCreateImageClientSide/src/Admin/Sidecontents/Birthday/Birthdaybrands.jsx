...
function Birthdaybrands() {
  ...
  useEffect(() => {
    fetch("https://icreate-admin-backend.onrender.com/birthday")
      .then((res) => res.json())
      .then((data) => setAllBirthday(data));
  }, [checkDelete]);

  const handleDelete = (id) => {
    fetch(`https://icreate-admin-backend.onrender.com/birthday/${id}`, {
      method: "DELETE",
    });
  ...
  function sendtoform(id, image, name) {
    navigate(`/Admin/birthday-sub-brands/${id}`);
  }
  ...
  return (
    <div>
      ...
      <div className="grid md:grid-cols-4 sm:grid-cols-2 gap-7">
            ...
            <div>
              ...
              <div className="flex justify-center items-center space-x-12 py-3">
                <div className="w-[30%] border-2 border-slate-700 flex items-center justify-center rounded-md bg-[#4BB543]">
                  <NavLink
                    to={`/Admin/birthdaybrandsedit/${item.design[0]._id}`}
                  >
                    <button>Edit</button>
                  </NavLink>
                </div>
        ...
        {allBirthday.length === 0 && <p className="text-center ">No Brands</p>}
      </div>
    </div>
  );
}
