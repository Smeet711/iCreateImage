...
function Birthdayaddnew() {
  ...
  const fetchBrandName = async () => {
    try {
      const response = await fetch("https://icreate-admin-backend.onrender.com/birthday/brandname");
  ...
  const submit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://icreate-admin-backend.onrender.com/birthday/new-design",
        formData,
        {
          headers: {
      ...
      if (response.data.msg === "Design added successfully") {
        ...
        setErrorMessage(false); // Clear any previous error message
         ...
         if (selectedBrand) {
           navigate(`/Admin/birthday-sub-brands/${selectedBrand._id}`);
         }
      ...
      <div className="w-[100%]">
        <div className=" w-[100%]">
          <div className="text-center font-extrabold text-5xl opacity-50  py-4">
            <h1>Admin Panel for Birthday</h1>
          </div>
            ...
            <div>
              {successMessage && (
                <p className="text-center text-green-500 text-xl  font-bold">
                  Birthday Design created successfully
                </p>
              )}

              {errorMessage && (
                <p className="text-center text-red-500 text-xl  font-bold">
                  Error creating the Birthday Design
                </p>
              )}
            </div>
          </div>
