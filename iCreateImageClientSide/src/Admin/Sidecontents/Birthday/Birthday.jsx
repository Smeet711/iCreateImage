...
function Birthday() {
  const [birthdayData, setData] = useState([]);
  const type = "birthdaygreeting";
  const [fromDate, setFromDate] = useState("");
  const [toDate, setToDate] = useState("");
  useEffect(() => {
    fetch("https://icreate-user-backend.onrender.com/api/birdthday-sub-brand")
      .then((res) => res.json())
        ...
        <div className="w-full">
          <div className="w-full">
            <div className="h-[135px] rounded-xl grid place-items-center bg-purple-700 text-white">
              <div>
                <h1 className="text-lg font-semibold">BIRTHDAY CARD</h1>
                <h1 className="text-4xl">{birthdayData.birthdayCount}</h1>
              </div>
            </div>
  
            <div className="grid grid-cols-3 pt-10 gap-4">
              {birthdayData.subBrands &&
                birthdayData.subBrands.map((item, index) => (
                  <div
                    key={index}
