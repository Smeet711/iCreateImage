...
function Reminder() {
  const [ReminderData, setData] = useState([]);
  const type = "reminder";
  const [fromDate, setFromDate] = useState("");
  const [toDate, setToDate] = useState("");

  useEffect(() => {
    fetch("https://icreate-user-backend.onrender.com/api/reminder-sub-brand")
      .then((res) => res.json())
        ...
        <div className="w-full">
          <div className="h-[135px] rounded-xl grid place-items-center bg-purple-700 text-white">
            <div>
              ...
              <h1 className="text-4xl">{ReminderData.reminderCount}</h1>
            </div>
          </div>

          <div className="grid grid-cols-3 pt-10 gap-4">
            {ReminderData.subBrands &&
              ReminderData.subBrands.map((item, index) => (
                <div
                  key={index}
