...
function PosterPreview3({ MRID, image, canvas, doctorName, topic, name }) {
    ...
    <>
      <div className="md:w-[794px] sm:h-[1123px]  h-[763px] m-auto">
        ...
        <div
          ...
          style={customstyles}
        >
          <div className="flex justify-between  relative bottom-[-404px]  ">

          
<div className="md:w-[400px] flex flex-col justify-center items-center relative md:left-[22px] md:top-[63px] left-[16px] top-[-176px] w-[200px] "> 
          <h1 className="md:text-3xl text-sm capitalize m-[5px] mb-[10px] font-bold text-blue-800 text-center">Dr. {doctorName}</h1>

          <h1 className="md:text-2xl text-sm font-bold text-black relative  right-[10px]">{topic}</h1>
          </div>
          <div className=" relative md:w-[399px] md:left-[-21px] md:top-[-110px] w-[190px] left-[-28px] top-[-237px] ">
            {canvas && (
              <img className="w-full rounded-full p-[50x]" src={canvas.toDataURL()} alt="IMAGE-DOCTOR" />
            )}
           </div>
          </div>
        </div>
